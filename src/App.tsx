...
type View = 'chat' | 'rankings' | 'overview' | 'journal' | 'replay' | 'watchlist' | 'settings' | 'methodology' | 'trading';
...
export default function App() {
  ...
  const navItems: { id: View; label: string; icon: typeof Activity; count?: number }[] = [
    ...
    { id: 'trading', label: 'Wallet & trading', icon: ShieldCheck },
  ];
  return <div className="app-shell">
    ...
    <div className="main-shell">
      ...
      <main>
        ...
        {marketError && view !== 'trading' && <div className="notice error-notice" role="alert"><Activity size={16} /><span>{symbol}: {marketError}</span></div>}
        {data?.errors.length && view !== 'trading' ? <div className="notice"><Radio size={16} /><span>Some feeds are unavailable: {data.errors.slice(0, 20).map(item => item.symbol).join(', ') + (data.errors.length > 20 ? ` and ${data.errors.length - 20} more` : '')}. Available markets are shown below.</span></div> : null}
        ...
        <div hidden={view !== 'trading'}><TradingDesk active={view === 'trading'} assets={assets} liveEnabled={health?.demo === false} /></div>
        ...
        <footer className="page-footer"><span><Logo small />Built for perspective, not certainty.</span><span>Research · Wallet-confirmed swaps <span className="footer-dot">·</span> <button onClick={() => navigate('methodology')}>Methodology <ArrowUpRight size={11} /></button></span></footer>
      </main>
    </div>
